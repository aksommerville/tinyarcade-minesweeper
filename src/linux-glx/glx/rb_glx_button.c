 ...
 * TODO Numeric keypad is not reporting correctly. Digits absent, and '.' comes in as DEL.
 */
...
int rb_glx_usb_usage_from_keysym(int keysym) {
  ...
  /* Letters are in order, keysyms match ASCII. Beware that X11 might report them upper or lower case. */
  if ((keysym>=0x41)&&(keysym<=0x5a)) return 0x00070004+keysym-0x41;
  if ((keysym>=0x61)&&(keysym<=0x7a)) return 0x00070004+keysym-0x61;
  
  /* 1..9 are ordered in USB; for X11 these are ASCII. Ditto for keypad. */
  ...
  switch (keysym) {
    ...
    // X11 "helpfully" turns the keypad into special-function keys...
