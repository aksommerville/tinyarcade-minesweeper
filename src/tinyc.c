...
//TODO DMA xfer, see ~/Arduino/libraries/TinyScreen/TinyScreen.cpp
...
static void writeRemap(void) {
  ...
  //NB This forces 8-bit BGR and no xform, see TinyScreen.cpp if we ever want otherwise.
  startCommand();
...
uint8_t tinyc_read_input() {
  uint8_t state=0;
  if (analogRead(42)<0x08) state|=TINYC_BUTTON_UP;
  if (analogRead(19)<0x08) state|=TINYC_BUTTON_DOWN;
  if (analogRead(25)<0x08) state|=TINYC_BUTTON_LEFT;
  if (analogRead(15)<0x08) state|=TINYC_BUTTON_RIGHT;
